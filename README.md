...
- Una **pieza de reto: la tuerca** — un anillo 3×3 con el centro hueco. El agujero atraviesa los bloques ya fijados, así que si la dejas caer sobre relleno sella una celda vacía imposible de completar. Sale con la misma probabilidad que las demás.
...
- **Piezas**: definidas como matrices cuadradas. Para rotar se calcula la transposición + reverso de filas (`rotateCW`). El índice `8` (`NUT_TYPE`) es la tuerca: su celda central vale `0`, y como `collide` solo mira las celdas no nulas, el agujero pasa por encima de bloques fijados y deja un hueco sellado. El círculo del agujero lo pinta `drawNutHole` para la pieza activa, la fantasma y la vista previa.
